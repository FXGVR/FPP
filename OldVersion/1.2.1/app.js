...
function loadMediaFromQuery() {
  ...
  const mediaUrl = videoUrl || imageUrl || params.get("url");
...
function attemptAutoplay(successMessage) {
  video.play().then(() => {
    ...
    video.play().then(() => {
      setStatus(`${successMessage} 浏览器要求静音自动播放，已自动静音。`, "success");
    }).catch(() => {
...
function setProjectionFromSource(width, height, sourceName) {
  ...
  const ratio = width / height;
  const normalizedName = normalizeProjectionName(sourceName);
  const isSquare = isRatioNear(ratio, 1);
  const isWide360 = isRatioNear(ratio, 2);
  let nextProjection = "flat";

  if (hasProjectionKeyword(normalizedName, ["_EAC", " EAC", "-EAC", ".EAC"])) {
    nextProjection = "eac";
  } else if (hasProjectionKeyword(normalizedName, ["_SBS", "_3D", "_LR", " SBS", " 3D", " LR", "-SBS", "-3D", "-LR"])) {
    nextProjection = "flat-sbs";
  } else if (hasProjectionKeyword(normalizedName, ["_TD", "_OU", " TD", " OU", "-TD", "-OU"])) {
  ...
  projectionSelect.value = nextProjection;
}
