...
function loadMediaFromQuery() {
  ...
  state.muteRequestedByQuery = isTruthyQueryValue(muteParam);
...
function attemptAutoplay(successMessage) {
  video.play().then(() => {
    ...
    const muteHint = muteInput.checked ? "" : " 如需无交互自动静音播放，可在 URL 加 isMute=true。";
    setStatus(`${successMessage} 浏览器需要你点击画面左下角播放按钮才能开始。${muteHint}`, "success");
  });
}
